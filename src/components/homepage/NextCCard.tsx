import { Text, View, Image, ScrollView, TouchableOpacity, Keyboard } from 'react-native';
...
const buildNextCCards = async(handlePress) => {
    ...
    try {
        ...
        for (let i = 0; i < 31; i++) {
                    ...
                    >
                        <View className="pl-4">
                            <Text className="text-2xl font-semibold">
                                {i== 0 ? "Today's Tasks: " + dayContacts.length : dayname +
                                    String(new Date(today.getTime() + i * 24 * 60 * 60 * 1000)).substring(4, 15)}
                            </Text>
                        </View>
                        <ScrollView className= 'space-y-2 h-[77%] mb-6' key={i}>
            ...
            
            {
                ...
                calendarcards.push(
                    <View className={'h-28 w-20 bg-gray-200 rounded-xl border-2'} key={i}>
                        <View className='h-20'>
